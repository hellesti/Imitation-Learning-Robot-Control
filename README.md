...
To start the robot, follow these steps:
1. Connect all loose cables under the table. The gripper is often disconnected, so make sure it is properly plugged in.
2. Swap the Ethernet cable: Unplug the gray Ethernet cable from the PC and replace it with the green Ethernet cable, which connects the PC to the robot.
3. Turn on the robot using the large power button on the UNIVERSAL ROBOTS screen.
4. Navigate to initialization by clicking "Go to initialization screen" twice.
5. Turn on the robot by clicking "ON". If this is not possible, check if any of the two emergency stop buttons are pressed. If so, release them by pulling them up.
7. Start the robot by clicking "START"
8. Log in to the PC.
